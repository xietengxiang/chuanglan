...
public class HttpFileServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
	...
	protected void messageReceived(ChannelHandlerContext ctx, FullHttpRequest req) throws Exception {
		if (!req.getDecoderResult().isSuccess()) {
			sendError(ctx, HttpResponseStatus.BAD_REQUEST); //BAD_REQUEST
			return;
		}
		...
		ctx.write(res);
		ChannelFuture f = ctx.write(new ChunkedFile(randomAccessFile, 0, randomAccessFile.length(), 8192),
	...
	private static void sendListing(ChannelHandlerContext ctx, File dir) {
		...
		buf.append("<html><head><tile>");
		buf.append(dirPath);
		buf.append("  目录：");
		buf.append("</tile></head><body>\r\n");
		buf.append("<h3>");
		buf.append(dirPath).append("  目录：");
		buf.append("</h3>\r\n");
		buf.append("<ul>");
		buf.append("<li>链接：<a href=\"../\">..</a>\r\n");
	...
	private static void setContentTypeHeader(HttpResponse response, File file){
		...
		String type = mimeTypesMap.getContentType(name);
